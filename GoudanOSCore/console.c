...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal){
	if(strcmp(cmdline, "about") == 0 && cons->sht != 0){
		cmd_about(cons);
...
int *gdn_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax){
	struct TASK *task = task_now();
	...
	if(edx == 1){
		...
		sht = (struct SHEET *) (ebx & 0xfffffffe);
		gdn_api_linewin(sht, eax, ecx, esi, edi, ebp);
		if((ebx & 1) == 0){
			sheet_refresh(sht, eax, ecx, esi + 1, edi + 1);
		}
	} else if( edx == 14){
		...
		for(;;){
			io_cli();
			if(fifo32_status(&task->fifo) == 0){ // fifo 没有数据
				if(eax != 0){
					task_sleep(task);
				} else {
					io_sti();
...
void gdn_api_linewin(struct SHEET *sht, int x0, int y0, int x1, int y1, int col)
{
	int i, x, y, len, dx, dy;
